...
while IFS= read -r account; do
    ...
    port=$(echo "$account" | jq -r '.port')  # 获取端口信息

    echo "开始执行第${count}个账号"
    echo "用户: $username, 登陆面板: $panel, 登录端口: $port"

    output=$(timeout 30 sshpass -p "$password" ssh -n -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -tt -p "$port" "$username@$panel" "ps -A; echo '登录成功'" 2>&1)
    ...
    login_results+=("$username:$panel:$port:$login_result")
